------------------------------------------------------------------------------------------------------
--																									--
--					Blocker desenvolvido por PFE Microsoft											--
--																									--
--  Objetivo: capturar problemas de performance de banco de dados									--
------------------------------------------------------------------------------------------------------

IF OBJECT_ID('tempdb..#spBlockerPfe') IS NOT NULL
	DROP PROCEDURE #spBlockerPfe
GO
CREATE PROCEDURE #spBlockerPfe(@timespan INT = 2)
AS
...
SET @endDate = DATEADD(minute,@timespan,GETDATE())

PRINT 'BLOCKER_PFE_SCRIPT_YUKON Script v9.0.14 (SQL2005)'
PRINT ''
...
BEGIN
	...
	IF @step % 12 = 0 
	BEGIN
		DECLARE @savePrevDate DATETIME 

		SET @savePrevDate = GETDATE()
...
SELECT 
	NULL AS 'sqlserver_start_time', -- YUKON(SQL2005)
...
SET @time = GETDATE()
PRINT ''
PRINT 'BLOCKER_PFE_BEGIN sys.master_files[SizeGrowth]'
-- VIEW ANY DEFINITION
SELECT 
...
PRINT 'BLOCKER_PFE_END sys.master_files[SizeGrowth] '  + convert(VARCHAR(12), datediff(ms,@time,getdate())) 
...
SELECT 
	...
	NULL AS 'query_hash', NULL AS 'query_plan_hash'
FROM sys.dm_exec_requests req
WHERE connection_id IS NOT NULL -- YUKON(SQL2005)
...
SELECT
	...
	mg.used_memory_kb, mg.required_memory_kb, mg.max_used_memory_kb, NULL AS 'ideal_memory_kb', 
...
SELECT 
	qrsem.resource_semaphore_id, qrsem.target_memory_kb, qrsem.max_target_memory_kb, qrsem.total_memory_kb, qrsem.available_memory_kb, qrsem.granted_memory_kb, qrsem.used_memory_kb, qrsem.grantee_count, qrsem.waiter_count, qrsem.timeout_error_count, qrsem.forced_grant_count, 
	1 AS 'pool_id'
...
DECLARE @pending_io_requests_summary INT 

SELECT @pending_io_requests_summary = 0
